...
export default function InvitesPage() {
  ...
  // ----------------------------------
  // USER HAS NO GROUP → SHOW PROMPT
  // ----------------------------------
  if (loading) {
  return <div className="p-10 text-center text-muted-foreground">Loading...</div>
}

// 🚨 FIX: stop rendering the tabs unless the group is ready
if (!group) {
  return (
  ...
  const handleInviteSubmit = async (data: { method: string; contact: string; role: string }) => {
    ...
    const { data: inserted } = await supabase.from("invites").insert(payload).select().single()
    if (inserted) {
